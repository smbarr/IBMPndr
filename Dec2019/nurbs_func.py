#!/usr/bin/env python3

import os, sys, math
import numpy as np

def u_x(x):
  #u = (0.5*(x-(1.0/99.0))/x)+(1.0/(x*198.0))*np.sqrt(9999*x**2+1)

  u1 = (0.5*(x-(1.0/99.0))/x)
  s = 9999*x**2+1
  u2 = s/50.0
  for i in range(6):
    u2 = 0.5*(u2+(s/u2))
  u = u1 + u2/(198.0*x)
  #u = 0.5+(0.8*x/(0.06+(x**2/0.08+5*x**2)))-1.4*x
  #u = 0.5+2*x/(0.852294+((8.3416*x**2)/1.39703))
  #u = 0.5+300*x/(1.0+((100000*x**2)/1.39703))
  #u = 0.5+(3.32*x/(0.69855+(9.0*x**2/(0.48205+(15.457*x**2/10.218)))))-0.14*x**3
  return u

def x_u(u):
  #x = (2.0*u-1.0)/((1.0-u)**2+u**2+200.0*u*(1.0-u))
  x = (-1.0*(1.0-u)**2+u**2)/((1.0-u)**2+200.0*u*(1.0-u)+u**2)
  #x = -1.0/(300.0*u) - 1.0/(300.0*(u-1.0))
  #x = -1.0/(20.0*u) - 1.0/(20.0*(u-1.0))
  return x

def y_u(u):
  #y = ((1.0-u)**2+u**2)/((1.0-u)**2+100.0*(2.0*(1.0-u)*u)+u**2)
  y = (1.0*(1.0-u)**2+u**2)/((1.0-u)**2+200.0*u*(1.0-u)+u**2)
  #y = 0.5/(5000.0*u**2) + 0.5/(5000.0*(u-1.0)**2)
  #y = 1.0/(300.0*u) - 1.0/(300.0*(u-1.0))
  #y = 1.0/(20.0*u) - 1.0/(20.0*(u-1.0))
  return y

x = np.linspace(-1,1,1000)
ab = np.abs(x)
y = np.zeros(1000)
#u = np.linspace(0.0,1.0,1000)
out = open("func.dat","w")
for n in range(len(x)):
  #x = x_u(u[n])
  #u = u_x(x[n])
  #y = y_u(u)
  u = u_x(x[n])
  y[n] = y_u(u)
  out.write("%e %e %e\n"%(x[n],y[n],ab[n]))
out.close()

mse = np.square(np.subtract(y,ab)).mean()
print(mse)
